...
import { WallpaperDTO, api } from '../api/bridge';
...
export default function WallpaperGrid({
  ...
  useEffect(() => {
    const toLoad = entries.slice(0, visible);
    toLoad.forEach(async (e) => {
      if (thumbCache[e.path]) return;
      try {
        const t = await api.thumbnailFor(e.path);
        if (t.thumbnail) {
          setThumbCache((prev) => ({ ...prev, [e.path]: t.thumbnail! }));
        }
      } catch { /* ignore */ }
    });
  }, [entries, visible]);
