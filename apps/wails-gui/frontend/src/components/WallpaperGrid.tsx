...
const CARD_HEIGHT = 176; // thumb (120px) + info padding (~56px)
const OVERSCAN = 2; // rows above/below viewport to pre-render
...
export default function WallpaperGrid({
  ...
  // Reset virtualizer + thumbnail queue when entries change.
  useEffect(() => {
    reset();
    virtualizer.scrollToIndex(0);
  }, [entries]);

  // Queue thumbnails only for visible + overscan rows.
  useEffect(() => {
  ...
  // Group entries into rows for the virtualizer.
  const rows = useMemo(() => {
