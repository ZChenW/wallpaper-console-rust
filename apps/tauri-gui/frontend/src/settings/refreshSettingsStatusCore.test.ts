...
test('refreshSettingsStatusCore skips omitted loaders', async () => {
  let thumbCalls = 0;
  const snapshot = await refreshSettingsStatusCore({
    thumbnailCacheStatus: async () => {
      thumbCalls += 1;
      return { dir: '/tmp', size: '0 B', entries: 0, failureEntries: 0, cleanupDays: 30 };
    },
  });
  assert.equal(thumbCalls, 1);
  assert.equal(snapshot.thumbCache?.entries, 0);
