...
import { resolveLibraryDisplay, resolveLibraryEmptyMessage } from './libraryDisplay.ts';
...
test('resolveLibraryEmptyMessage distinguishes no sources', () => {
  assert.equal(
    resolveLibraryEmptyMessage({
      configured: 'sqlite',
      effective: 'sqlite',
      sqliteReady: true,
      sqliteRows: 0,
...
test('resolveLibraryEmptyMessage distinguishes stale legacy rows', () => {
  assert.equal(
    resolveLibraryEmptyMessage({
      configured: 'sqlite',
      effective: 'sqlite',
    ...
    'Library index is out of sync. Rebuild the SQLite index.',
  );
});

test('resolveLibraryEmptyMessage distinguishes empty index with configured sources', () => {
  assert.equal(
    resolveLibraryEmptyMessage({
      configured: 'sqlite',
      effective: 'sqlite',
    ...
    'Library index is empty. Rescan or repair the database.',
  );
});
