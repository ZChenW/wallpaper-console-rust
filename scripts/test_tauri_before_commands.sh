...
# Extract commands from JSON config (simple sed approach, no jq dependency).
extract_command() {
  local key="$1"
...
test_root_detection() {
  ...
  local output
  output=$(cd "$cwd" && timeout "$timeout_sec" sh -c "$command" 2>&1) || true
  local rc=$?
...
BUILD_SMOKE="while [ ! -d apps/tauri-gui/frontend ] && [ \"\$PWD\" != / ]; do cd ..; done; test -d apps/tauri-gui/frontend"
