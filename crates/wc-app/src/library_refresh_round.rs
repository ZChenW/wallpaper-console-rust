//! Deep module for one coherent Library refresh round.
//!
//! Callers select sources, state intent, observe progress, and may cancel. This
//! module owns eligibility, source locks, attempt state, scan reconciliation,
//! cancellation repair, and the optional legacy TSV projection.
...
/// Why a round was requested. Manual work bypasses freshness and retry
/// eligibility, while background work observes both and never waits on a busy
/// source.
...
/// Stable source selection for a single refresh round.
...
/// The one legacy projection result published by a successful refresh round.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum LegacyProjectionStatus {
    Published { rows: usize },
    Degraded { message: String },
}

/// Complete observable result of one refresh round.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LibraryRefreshRoundReport {
    pub selected_sources: usize,
    pub refresh: LibraryRefreshReport,
...
fn cancellation_error(
    ...
    if let Err(error) = wc_storage::sqlite::cancel_source_refresh_attempt(&storage.cd, source.id) {
        return storage_error(error, Some(&source), &report);
    }
    LibraryRefreshError::Cancelled {
    ...
    for source in sources {
        ...
        match outcome {
            ...
            SourceScanOutcome::Cancelled(stats) => {
                ...
                return Err(cancellation_error(storage, source, stats, report));
            }
        }
    }
...
fn selected_source_records(
    storage: &StorageApi,
    selection: RefreshSelection,
) -> Result<Vec<SourceRecord>, LibraryRefreshError> {
    let empty_report = LibraryRefreshReport::default();
    let sources = storage
        .source_records()
        .map_err(|error| storage_error(error, None, &empty_report))?;
    let RefreshSelection::Sources(requested) = selection else {
        return Ok(sources);
    };
    ...
    if selected.len() != requested.len() {
        ...
        return Err(storage_error(
            WcError::Other(format!("unknown source id(s): {missing:?}")),
            None,
            &empty_report,
        ));
    }
...
fn storage_intent(intent: RefreshIntent) -> wc_storage::sqlite::RefreshIntent {
    match intent {
        RefreshIntent::Manual => wc_storage::sqlite::RefreshIntent::Manual,
        RefreshIntent::Background => wc_storage::sqlite::RefreshIntent::Background,
    }
}

/// Run a complete refresh round and expose exactly one legacy projection
/// result. A projection failure leaves SQLite authoritative and is reported as
/// degraded success rather than changing the scan outcome.
pub fn run_library_refresh_round_with_clock<F, C>(
    storage: &StorageApi,
    selection: RefreshSelection,
    intent: RefreshIntent,
    clock: C,
    callback: F,
) -> Result<LibraryRefreshRoundReport, LibraryRefreshError>
where
    F: FnMut(&SourceRecord, &SourceScanEvent) -> ScanControl,
    C: FnMut() -> i64,
{
    let sources = selected_source_records(storage, selection)?;
    crate::library_rescan::establish_library_dirty_marker(storage).map_err(|error| {
        storage_error(WcError::Io(error), None, &LibraryRefreshReport::default())
    })?;
    let selected_sources = sources.len();
    let refresh_started = std::time::Instant::now();
    let refresh = refresh_selected_sources_with_clock(
        storage,
        sources,
    ...
    let library_rows = wc_storage::sqlite::source_backed_library_count(&storage.cd)
        .map_err(|error| storage_error(error, None, &refresh))?;
...
pub fn run_library_refresh_round<F>(
    storage: &StorageApi,
    selection: RefreshSelection,
    intent: RefreshIntent,
    callback: F,
...
where
    F: FnMut(&SourceRecord, &SourceScanEvent) -> ScanControl,
{
    run_library_refresh_round_with_clock(storage, selection, intent, system_unix_seconds, callback)
}
...
mod tests {
    ...
    use super::{
        ...
        refresh_library_sources_background_with_clock, run_library_refresh_round,
        LegacyProjectionStatus, LibraryRefreshError, RefreshIntent, RefreshSelection,
    };
    ...
    fn cancellation_preserves_current_snapshot_and_availability() {
        ...
        let refresh_state =
            wc_storage::sqlite::read_source_refresh_state(&storage.cd, source.id).unwrap();
        assert!(refresh_state.dirty);
        assert_eq!(refresh_state.failure_category, None);
        assert_eq!(refresh_state.consecutive_failures, 0);
        assert_eq!(refresh_state.next_retry_at, None);
    }
    ...
    #[test]
    fn round_reports_tsv_failure_as_degraded_sqlite_success() {
        let (_tmp, storage) = storage();
        fs::remove_file(storage.cd.library_tsv_path()).unwrap();
        fs::create_dir_all(storage.cd.library_tsv_path()).unwrap();

        let report = run_library_refresh_round(
            &storage,
            RefreshSelection::All,
            RefreshIntent::Manual,
            |_, _| ScanControl::Continue,
        )
        .expect("projection failure must not change the refresh outcome");

        assert_eq!(report.selected_sources, 0);
        assert_eq!(report.library_rows, 0);
        assert!(matches!(
            report.projection,
            LegacyProjectionStatus::Degraded { .. }
        ));
        assert!(crate::library_rescan::library_dirty_marker_path(&storage).exists());
    }

    #[test]
    fn round_rejects_unknown_source_selection_before_scanning() {
        let (_tmp, storage) = storage();

        let error = run_library_refresh_round(
            &storage,
            RefreshSelection::Sources(vec![4242]),
            RefreshIntent::Manual,
            |_, _| panic!("unknown source must not be scanned"),
        )
        .unwrap_err();
