...
pub(crate) mod test_support {
    ...
    pub(crate) struct FakeRuntime {
        ...
        pub failed_mpvpaper_launch_cleanup_error: Option<String>,
        pub lwe_apply_calls: Vec<(String, Vec<String>)>,
        pub lwe_apply_error: Option<String>,
        /// When set, awww stop_checked verification fails (socket stays Ready).
        pub stop_awww_error: Option<String>,
        /// When set, mpvpaper stop leaves a sentinel pid so driver stop_checked fails.
        pub stop_mpvpaper_error: Option<String>,
        pub running_swaybg_pids: Vec<u32>,
        pub swaybg_ready_pid: Option<u32>,
        pub swaybg_readiness_error: Option<String>,
        pub swaybg_wait_count: usize,
    ...
    impl Default for FakeRuntime {
        fn default() -> Self {
            Self {
                ...
                failed_mpvpaper_launch_cleanup_error: None,
                lwe_apply_calls: Vec::new(),
                lwe_apply_error: None,
                stop_awww_error: None,
                stop_mpvpaper_error: None,
                running_swaybg_pids: Vec::new(),
                swaybg_ready_pid: None,
                swaybg_readiness_error: None,
                swaybg_wait_count: 0,
    ...
    impl ProcessIo for FakeRuntime {
        ...
        fn command_status(
            ...
            let args: Vec<String> = command
                ...
                .map(|arg| arg.to_string_lossy().to_string())
                .collect();
            if program == "awww" && args.first().map(String::as_str) == Some("clear") {
                self.clear_awww_state_hint_count += 1;
            }
            self.command_status_programs.push(program);
            self.command_status_args.push(args);
        ...
        fn swaybg_pids(&mut self) -> Result<Vec<u32>, WcError> {
            Ok(self.running_swaybg_pids.clone())
        }
        ...
        fn wait_for_swaybg_ready(
            ...
            self.swaybg_wait_count += 1;
            match &self.swaybg_readiness_error {
                Some(message) => Err(WcError::Other(message.clone())),
                None => Ok(self.swaybg_ready_pid.unwrap_or(1)),
            }
        }

        fn swaybg_pid_running(&mut self, pid: u32) -> Result<bool, WcError> {
            self.swaybg_pid_running_checks.push(pid);
            Ok(true)
        }
    ...
    impl BackendRuntime for FakeRuntime {
        ...
        fn stop_mpvpaper(&mut self) {
            ...
            if self.stop_mpvpaper_error.is_some() {
                if self.running_mpvpaper_pids.is_empty() {
                    self.running_mpvpaper_pids.push(1);
                }
            } else {
        ...
        fn stop_swaybg(&mut self) {
            self.stop_swaybg_count += 1;
            self.running_swaybg_pids.clear();
        }
