...
| Wallpaper Engine scene/Web backend | Completed (Scene), Preview-only (Web) | Scene (`we_scene`): optional external `linux-wallpaperengine` with compat cache for incompatibility detection. Web (`we_web`): indexed and displayed with preview GIF; formal apply returns `web_renderer_unavailable`. Chromium preview is experimental debug/browser preview only (not a real desktop wallpaper). Native WebKitGTK layer-shell renderer pending. |
