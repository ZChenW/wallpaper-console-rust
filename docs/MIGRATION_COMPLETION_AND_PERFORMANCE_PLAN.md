...
## Current Verdict (updated 2026-06-10)

The Rust migration has been executed as a **side-by-side Rust CLI + Wails GUI replacement candidate**.

Current supported path:

- `wallpaper-console-rust` — Rust CLI (50+ commands, all parity gaps closed except `tui`)
- `wallpaper-console-gui-rust` — Wails v3 + React GUI backed by the Rust CLI

### What's Complete

All major phases (0-10) are complete:

- **CLI parity**: All 50+ commands including `__preview__`, all 6 SQLite debug commands, Flatpak Steam paths
- **Performance**: Paginated SQLite library, thumbnail queue (concurrency 2), virtualized grid (@tanstack/react-virtual), incremental rescan with metadata cache, video thumbnail v2 (multi-point smart frame sampling)
- **Wails GUI**: 34 bound methods (33 user-facing + Binary helper) via generated bindings (NOT `window.wails.Call`), all 5 views, light theme, settings safety (verify-before-switch storage mode)
- **Tauri GUI**: Compiles and builds `.deb`/`.rpm` bundles. Calls Rust crates directly (no subprocess). Asset protocol configured for local thumbnail loading. Experimental but functional.
- **install.sh**: Side-by-side installation with `-rust` suffix, temp config smoke test, proper shell safety

### What Remains

- `tui` is still a stub — Wails/Tauri GUI is the intended replacement for the old Bash control TUI
- Phase 11 (niri integration): needs real-use validation
- Phase 12 (deprecate Bash/Python): deferred until real-use validation passes
- Tauri: needs real-use smoke testing before becoming the default GUI

### Key Architecture Facts (for AI/agentic workers)

1. **Rust crates** (`wc-core`, `wc-storage`, `wc-scan`, `wc-backend`, `wc-preview`, `wc-cli`) are the source of truth for all business logic
2. **Wails bridge** calls Rust CLI as subprocess — thin, simple, keeps Rust testable independently
3. **Wails uses generated bindings** at `apps/wails-gui/bindings/wallpaper-console-gui/bridge.ts` — never `window.wails.Call`
4. **Tauri bridge** calls Rust crates directly via `#[tauri::command]` in `apps/tauri-gui/src-tauri/src/commands.rs`
5. **Thumbnails** are generated by Rust CLI `thumbnail` subcommand, not by Go. Video thumbnails use v2 smart sampling (25%/50%/10%/5s/75% positions, content detection via ImageMagick fx:mean/stddev)
6. **Storage** supports 3 modes: `file`, `hybrid` (dual-write), `sqlite` (primary). Config key: `storage_backend`
7. **Config dir**: `$XDG_CONFIG_HOME/wallpaper-console` (default `~/.config/wallpaper-console`)
8. **54 tests** total: 35 wc-cli integration + 7 wc-core + 12 wc-scan
9. **Tauri asset protocol** fix: `features = ["protocol-asset"]` in Cargo.toml + `assetProtocol` scope in tauri.conf.json allowing `$HOME/.config/wallpaper-console/cache/gui-thumbnails/**`
10. **Do not modify** the original Bash/Python `wallpaper-console` repo — this Rust repo is in `/home/chakew/Projects/wallpaper-console-rust`

Do not deprecate the Bash/Python implementation until every acceptance gate in this plan passes.

---

## Evidence From Repository Audit (historical — most issues now resolved)
